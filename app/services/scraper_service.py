...
def _next_proxy(proxies: List[Dict[str, str]]):
    global _PROXY_INDEX
    if not proxies:
        return None
...
async def scrape_single_url(browser: Browser, scanner: ScannerConfig, proxies: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    for attempt in range(settings.retry_attempts):
        ...
        try:
            ...
            scraped_rows = await _extract_data_from_pages(page)
...
async def run_scrapers(scanners: List[ScannerConfig], proxies: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    async with async_playwright() as p:
        browser = await p.firefox.launch()

        semaphore = asyncio.Semaphore(settings.max_concurrent_scanners)
