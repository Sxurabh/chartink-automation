...
class SheetsService:
    ...
    def _get_or_create_worksheet(self) -> gspread.Worksheet:
        try:
            ...
            # 👇 Corrected settings.works_heet_name to settings.worksheet_name
    ...
    def get_existing_stocks_for_table(self, table_index: int) -> List[List[str]]:
        """Helper to get existing stocks for a specific table."""
        table_defs = [{'start': 'A', 'end': 'G'}, {'start': 'J', 'end': 'P'}]
        if table_index >= len(table_defs):
            return []
        ...
        try:
            ...
            if len(existing_values) > 2:
                return [row for row in existing_values[2:] if any(row)]
            else:
                return []
